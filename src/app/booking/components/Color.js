...
export default function Color({ color, isActive, onClick }) {
    return (
        <div
            className={`flex flex-col items-center m-2 cursor-pointer ${isActive ? 'scale-110' : ''}`}
            onClick={onClick}
            tabIndex={0} // Makes the div focusable
        >
            <div className='h-44 w-12 rounded-full' style={{ backgroundColor: color }}></div>
            <div>
                <span style={{ color: '#333', fontSize: '10px', marginTop: '4px', textAlign: 'center' }}>
                    {color}
                </span>
            </div>
