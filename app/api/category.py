...
@role_required("admin")
def update_category(category_id: int, updated_data: CategoryUpdate, db: Session = Depends(get_db),  current_user: dict = Depends(get_current_user)):
...
def get_all_categories_with_services(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user) ):
